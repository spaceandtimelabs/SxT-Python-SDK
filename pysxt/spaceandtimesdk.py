...
import base64
import binascii
from pysxt.validation import (
 validate_number, validate_string, validate_list, is_base64, check_prefix_and_joincode, try_parse_identifier
)
...
import time
import os
from biscuit_auth import KeyPair, BiscuitBuilder, Fact, PublicKey, PrivateKey
from nacl.signing import SigningKey, VerifyKey
from nacl.exceptions import BadSignatureError
from dotenv import load_dotenv, set_key, get_key

from pysxt.keygen import exported_keys
...
class SpaceAndTimeSDK:
    ...
    def generate_auth_code(self, user_id, prefix, join_code):
        try:
            check_prefix_and_joincode(prefix, join_code)
    ...
    def generate_tokens(self, user_id, auth_code, private_key, public_key, scheme="ed25519"): 
        ...
        try:
            is_base64(private_key)
            is_base64(public_key)
    ...
    def authenticate(self, priv_key, pub_key, prefix=""):
        ...
        # Scheduler for Automatic token rotation
        schedule.every(25).minutes.do(self.refresh_token)
        while True:
    ...
    def get_tables(self, scope, namespace):
        try:
            ...
            try_parse_identifier(namespace)
            tokens = self.read_file_contents()
            access_token = tokens["accessToken"]
    ...
    def discovery_API_Request(self, namespace, table_name, api_endpoint):
         try:
           try_parse_identifier(namespace)
           try_parse_identifier(table_name)
           tokens = self.read_file_contents()
           access_token = tokens["accessToken"]
    ...
    def generate_biscuits(self, private_key, operations, resource_ids, wildcard_required = False):
        resource_ids_list = []
        for resource_id in resource_ids: resource_ids_list.append(resource_id.lower())

        biscuit_builder = BiscuitBuilder("")
        ...
        sql_operations = []
        for operation in operations: sql_operations.append(operation_capabilities[operation.upper()])

        biscuit_tokens = []
        if wildcard_required == True:
            for resource_id in resource_ids_list:
                biscuit_builder = BiscuitBuilder("")
                wildcard = "*"

                biscuit_private_key = PrivateKey.from_hex(private_key)
                biscuit_builder.add_fact(Fact(""" sxt:capability({wildcard}, {resource_id})""", { 'wildcard':wildcard, 'resource_id':resource_id }))
                biscuit_token = biscuit_builder.build(biscuit_private_key).to_base64()
                biscuit_tokens.append(biscuit_token)
            ...
            for resource_id in resource_ids_list: 
                ...
                for operation in sql_operations:
                    ...
                    biscuit_builder.add_fact(Fact(""" sxt:capability({operation}, {resource_id})""", { 'operation':operation, 'resource_id':resource_id }))
                    biscuit_token = biscuit_builder.build(biscuit_private_key).to_base64()

                biscuit_tokens.append(biscuit_token)
            
        return biscuit_tokens

    # Create a Schema
    def CreateSchema(self, sql_text, biscuit_tokens=[], origin_app=""):
        try:
            validate_string(sql_text)
    ...
    def DDLCreateTable(self, sql_text, access_type, public_key, biscuit_tokens=[], origin_app=""):
        try:
           validate_string(sql_text)
           validate_string(access_type)
           validate_list(biscuit_tokens)
           ...
           return {"response" : response.text, "error" : None}
            ...
            return {"response" : None, "error" : (str(response.text)).replace("\n", " ")}

        except TypeError as error:
            return {"response" : None, "error" : str(error)}

    # Alter and drop a table with the given ResourceId
    def DDL(self, sql_text, biscuit_tokens=[], origin_app=""):
        try:
            validate_string(sql_text)
            validate_list(biscuit_tokens)
            tokens = self.read_file_contents()
    ...
    def DML(self, resource_id, sql_text, biscuit_tokens=[], origin_app=""):
        try:
            try_parse_identifier(resource_id)
            validate_list(biscuit_tokens)
            validate_string(sql_text)
            tokens = self.read_file_contents()
            access_token = tokens["accessToken"]
    ...
    def DQL(self, resource_id, sql_text, biscuit_tokens=[], origin_app="", row_count=0):
        try:
            try_parse_identifier(resource_id)
            validate_list(biscuit_tokens)
            validate_string(sql_text)
