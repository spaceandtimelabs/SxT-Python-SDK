...
# Generate Biscuits
biscuit_tokens = SpaceAndTimeInit.generate_biscuits(private_key, ["CREATE", "ALTER", "DROP", "INSERT", "UPDATE", "MERGE", "DELETE", "SELECT"], ["ETH.TESTTABLE1", "ETH.TESTTABLE2"],  False)
print(biscuit_tokens)

# DDL
# Create a table
ddl_create_table_data = SpaceAndTimeInit.DDLCreateTable(create_sql_text, access_type, main_public_key, biscuit_array)
...
dml_data = SpaceAndTimeInit.DML(insert_sql_text, biscuit_array)
print(dml_data)

# DQL
# Select query and selects all if row_count = 0
dql_data = SpaceAndTimeInit.DQL(resource_id, select_sql_text, biscuit_array)
print(dql_data)
